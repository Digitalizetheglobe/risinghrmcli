    ...
    if (\Auth::user()->can('Create Leave')) {
        ...
        if (Auth::user()->type == 'employee') {
            ...
            if ($employees) {
                ...
                $compOffBalance = \DB::table('comp_off_leaves')
    ...
    public function store(Request $request)
{
    if (\Auth::user()->can('Create Leave')) {
        ...
        $isCompOff = $leave_type->title === 'Comp-Off';
        $isCasualLeave = $leave_type->unlimited; // Check if it's an unlimited casual leave

        // For Comp-Off and Casual Leave, skip the available days check
        ...
        $leave->total_leave_days = $isCompOff ? 1 : $total_leave_days; // Comp-Off is always 1 day
        ...
        // Deduct comp-off if this is a comp-off leave
        if ($isCompOff) {
    ...
    {
        $balance = \DB::table('comp_off_leaves')
            ->where('employees_id', $employeeId)
            ->count();

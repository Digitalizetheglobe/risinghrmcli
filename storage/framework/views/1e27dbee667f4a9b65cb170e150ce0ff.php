...
<div class="modal-body">
    ...
    <p id="comp-off-balance-display">Comp-Off Balance: <?php echo e($compOffBalance); ?></p>
                ...
                <select name="leave_type_id" id="leave_type_id" class="form-control select">
                    ...
                    <?php $__currentLoopData = $leavetypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $leave): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php if($leave->title === 'Comp-Off'): ?>
                            <?php if($compOffBalance > 0 || \Auth::user()->type != 'employee'): ?>
                                <option value="<?php echo e($leave->id); ?>" 
                                        data-is-comp-off="1"
    ...
    $(document).ready(function() {
        ...
        function updateCompOffDisplay(balance) {
            ...
            if ($compOffOption.length) {
                ...
                if (<?php echo json_encode(Auth::user()->type != 'employee', 15, 512) ?>) {
                    $compOffOption.show();
                } else {
                    // For employees, only show if balance > 0
                    $compOffOption.toggle(balance > 0);
                }
            } else if (balance > 0 || <?php echo json_encode(Auth::user()->type != 'employee', 15, 512) ?>) {
                // If comp-off option doesn't exist but should be shown
                var compOffLeave = <?php echo json_encode($leavetypes->firstWhere('title', 'Comp-Off'), 512) ?>;
                if (compOffLeave) {
        ...
        updateCompOffDisplay(<?php echo e($compOffBalance); ?>);
...
$(document).on('change', '#start_date, #end_date', function() {
    ...
    if (startDate && endDate) {
        ...
        if (leaveTypeId) {
            ...
            $.ajax({
                url: '<?php echo e(route('get.leave.type.details')); ?>',
                type: 'POST',
                data: {
...
</script><?php /**PATH C:\xampp\htdocs\hrm\resources\views/leave/create.blade.php ENDPATH**/ ?>